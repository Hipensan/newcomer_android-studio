package com.example.normalsettings;
...
public class SettingPreferenceFragment extends PreferenceFragment {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState){
        ...
        keywordSoundPreference = (ListPreference)findPreference("keyword_list");
        keywordScreen = (PreferenceScreen)findPreference("keyword_screen");

        prefs = PreferenceManager.getDefaultSharedPreferences(getActivity());

        if(!prefs.getString("sound_list", "").equals("")){
            soundPreference.setSummary(prefs.getString("sound_list", "1번"));

        }

        if(!prefs.getString("keyword_sound_list","").equals("")){
            keywordSoundPreference.setSummary(prefs.getString("keyword_list", "1번"));
